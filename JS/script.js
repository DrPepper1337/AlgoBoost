function generateStrings(nameOfElement, nOfString) {
    let newSt = "", st = "";
    while (context.measureText(st).width < window.screen.width * 2) {
        let listing = document.getElementById(nameOfElement + nOfString);
    ...
    console.log(`${nOfString} ` + context.measureText(st).width);
}
...
for (let i = 1; i < 13; i++) {
    if (i % 2 === 1) {
        elem.innerHTML = elem.innerHTML + `<div class="listingsOfCode" id="code${i}"><span id="listing${i}" class="forward"></span><span id="delayListing${i}" class="forward-delay"></span></div>`;
    } else {
        elem.innerHTML = elem.innerHTML + `<div class="listingsOfCode" id="code${i}"><span id="listing${i}" class="backwards"></span><span id="delayListing${i}" class="backwards-delay"></span></div>`;
    }
    generateStrings("listing", i);
    generateStrings("delayListing", i);
}
